    ...
    <div class="bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Editar Tutorial</h3>
            ...
            <form action="{{ route('admin.tutoriais.update', $tutorial) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $tutorial->titulo) }}" required
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="descricao" id="descricao" rows="3" 
                                  ...
                                  placeholder="Descreva brevemente o conteúdo do tutorial...">{{ old('descricao', $tutorial->descricao) }}</textarea>
                        @error('descricao')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    @if($tutorial->arquivo_video)
                        <div class="lg:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Vídeo Atual</label>
                            <div class="bg-gray-50 rounded-lg p-4">
                                <video controls class="w-full max-w-md h-48 rounded-lg">
                    ...
                    <div class="lg:col-span-2">
                        <label for="arquivo_video" class="block text-sm font-medium text-gray-700">
                            {{ $tutorial->arquivo_video ? 'Substituir Vídeo (MP4)' : 'Arquivo de Vídeo (MP4) *' }}
                        </label>
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600">
                                    <label for="arquivo_video" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                        <span>{{ $tutorial->arquivo_video ? 'Envie um novo arquivo' : 'Envie um arquivo de vídeo' }}</span>
                                        <input id="arquivo_video" name="arquivo_video" type="file" class="sr-only" accept=".mp4">
                                    </label>
                    ...
                    <div>
                        ...
                        <select name="publico_alvo" id="publico_alvo" required
                            ...
                            <option value="colaboradores" {{ old('publico_alvo', $tutorial->publico_alvo) == 'colaboradores' ? 'selected' : '' }}>Colaboradores</option>
                            <option value="clientes" {{ old('publico_alvo', $tutorial->publico_alvo) == 'clientes' ? 'selected' : '' }}>Clientes</option>
                        </select>
                        @error('publico_alvo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="ordem" id="ordem" value="{{ old('ordem', $tutorial->ordem) }}" min="0"
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex items-center">
                            <input type="checkbox" name="ativo" id="ativo" value="1" {{ old('ativo', $tutorial->ativo) ? 'checked' : '' }}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateFileName(fileName) {
        ...
        textElement.textContent = `Novo arquivo selecionado: ${fileName}`;
    }
});
</script>
@endsection