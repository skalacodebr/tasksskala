        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div>
                @if($conta->status == 'pendente')
                    <button onclick="abrirModalPagar()" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mr-2">
                        Pagar
                    </button>
                @endif
                <a href="{{ route('admin.contas-pagar.edit', $conta->id) }}" class="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-2">
                    Editar
                </a>
                <a href="{{ route('admin.contas-pagar.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                    Voltar
                </a>
            </div>
        </div>
        ...
        <div class="card-dark shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="border-t border-gray-700">
                <dl>
                    ...
                    <div class="card-dark px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-muted-dark">Data de Pagamento</dt>
                        <dd class="mt-1 text-sm text-primary-dark sm:mt-0 sm:col-span-2">{{ $conta->data_pagamento->format('d/m/Y') }}</dd>
                    </div>
                    @endif
                    <div class="bg-gray-800 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    @if($conta->fornecedor)
                    <div class="card-dark px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-muted-dark">Fornecedor</dt>
                        <dd class="mt-1 text-sm text-primary-dark sm:mt-0 sm:col-span-2">{{ $conta->fornecedor }}</dd>
                    </div>
                    @endif
                    @if($conta->categoria)
                    <div class="bg-gray-800 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
        ...
        <div class="inline-block align-bottom card-dark rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form action="{{ route('admin.contas-pagar.pagar', $conta->id) }}" method="POST">
                ...
                <div class="card-dark px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-primary-dark mb-4">Confirmar Pagamento</h3>
                    <div class="mb-4">
                        <label class="block text-muted-dark text-sm font-bold mb-2">Data do Pagamento</label>
                        <input type="date" name="data_pagamento" required class="shadow appearance-none border rounded w-full py-2 px-3 text-muted-dark leading-tight focus:outline-none focus:shadow-outline" value="{{ date('Y-m-d') }}">
                    </div>
                    <div class="mb-4">
                ...
                <div class="bg-gray-800 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button" onclick="fecharModalPagar()" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-600 shadow-sm px-4 py-2 card-dark text-base font-medium text-muted-dark hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                        Cancelar
                    </button>
                </div>
            </form>
