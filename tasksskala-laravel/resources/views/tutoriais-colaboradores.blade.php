    ...
    @if($tutoriais->count() > 0)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($tutoriais as $tutorial)
                <div class="card-dark shadow rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                    <!-- Thumbnail do Vídeo -->
                    <div class="relative bg-gray-900 aspect-video cursor-pointer" onclick="playVideo('video-{{ $tutorial->id }}')">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 hover:bg-opacity-20 transition-all duration-300"
                            ...
                            <div class="card-dark bg-opacity-90 rounded-full p-4 hover:bg-opacity-100 transition-all duration-300">
                    ...
                    <!-- Conteúdo do Card -->
                    <div class="p-4">
                        <h3 class="text-lg font-semibold text-primary-dark mb-2">{{ $tutorial->titulo }}</h3>
                        
                        @if($tutorial->descricao)
                            <p class="text-gray-400 text-sm mb-3 line-clamp-3">{{ $tutorial->descricao }}</p>
                        @endif
                        
                        <!-- Controles do Vídeo -->
                        <div class="flex items-center justify-between text-sm text-muted-dark">
                            ...
                            <div class="flex items-center space-x-2">
                                <button onclick="toggleFullscreen('video-{{ $tutorial->id }}')" 
                                        class="text-gray-400 hover:text-gray-400" title="Tela cheia">
    ...
    @else
        ...
        <div class="card-dark shadow rounded-lg">
            <div class="px-4 py-8 sm:p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-primary-dark mb-2">Nenhum tutorial disponível</h3>
                <p class="text-gray-400">Não há tutoriais disponíveis para colaboradores no momento.</p>
            </div>
        </div>
    @endif
</div>
