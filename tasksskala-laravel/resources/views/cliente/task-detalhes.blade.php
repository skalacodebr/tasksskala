...
@section('title', 'Solicitação #' . $task->id)

@section('content')
<div class="px-4 sm:px-6 lg:px-8">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('cliente.minhas-tasks') }}" class="inline-flex items-center text-purple-600 hover:text-purple-700 transition-colors">
    ...
    <div class="bg-gradient-to-r from-purple-600 to-pink-600 rounded-2xl p-8 text-white mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-purple-100">Detalhes da sua solicitação de correção ou ajuste</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-6">
                    <!-- Repository -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Repositório do Projeto</label>
                    ...
                    <!-- Task Description -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Descrição da Solicitação</label>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <div class="p-3 {{ $task->status === 'aprovado' ? 'bg-green-50' : ($task->status === 'pendente' ? 'bg-yellow-50' : 'bg-red-50') }} rounded-lg">
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-gray-600">Status Atual:</span>
                            <span class="font-semibold {{ $task->status === 'aprovado' ? 'text-green-600' : ($task->status === 'pendente' ? 'text-yellow-600' : 'text-red-600') }}">
                                {{ ucfirst($task->status) }}
                            </span>
                        </div>
                    </div>
                    @if($task->plans && $task->plans->count() > 0)
                        <div class="p-3 bg-blue-50 rounded-lg">
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-600">Planos Gerados:</span>
                                <span class="font-semibold text-blue-600">{{ $task->plans->count() }}</span>
                            </div>
                        </div>
                        <div class="p-3 bg-purple-50 rounded-lg">
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-600">Planos Aprovados:</span>
                                <span class="font-semibold text-purple-600">{{ $task->plans->where('approved', true)->count() }}</span>
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <a href="{{ route('cliente.minhas-tasks') }}" class="w-full inline-flex items-center justify-center px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path>
                        </svg>
                        Ver Todas Solicitações
                    </a>
                    <a href="{{ route('cliente.criar-task') }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mt-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-xl font-semibold text-gray-900 mb-6 flex items-center">
                ...
                Planos de Execução ({{ $task->plans->count() }})
            </h3>

            <div class="space-y-6">
                @foreach($task->plans as $index => $plan)
                    <div class="border border-gray-200 rounded-xl p-6 {{ $plan->approved ? 'bg-green-50 border-green-200' : 'bg-gray-50' }} transition-all">
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="text-lg font-semibold text-gray-900 flex items-center">
                                <span class="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center text-white text-sm font-bold mr-3">
                                    {{ $index + 1 }}
                                </span>
                                Plano #{{ $plan->id }}
                            </h4>
    ...
    <div class="mt-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">Nenhum plano gerado ainda</h3>
            <p class="text-gray-500">Nossa equipe ainda está analisando sua solicitação e criando o plano de execução.</p>
        </div>
    </div>
    @endif
</div>
